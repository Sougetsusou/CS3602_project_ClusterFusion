...
> Scope note: our reproduction is **forward-only** and implemented in **pure PyTorch** (no custom CUDA). Despite relying solely on standard PyTorch operators, it faithfully matches FlashAttention’s mathematics and memory profile, providing a transparent reference implementation that is easy to read, verify, and extend.
...
- MiniFlash preserves quality (PPL matches baseline) and delivers the same flat-memory advantage as FlashAttention-2, demonstrating the correctness of the algorithmic design even without specialized kernels.
...
- MiniFlash vs eager:
  - confirms the algorithmic benefit (memory-constant), albeit without the fused-kernel optimizations that FlashAttention-2 employs
...
### 5.2 Internal efficiency considerations for MiniFlash
MiniFlash intentionally prioritises *clarity* over raw speed: it showcases the algorithm in ~200 lines of plain PyTorch so that every step can be inspected and modified. This design choice leaves several well-known avenues for future optimisation:
- fuse the per-block matmuls into a custom CUDA kernel
- batch blocks to reduce kernel-launch overhead
- apply operator fusion for the online-softmax updates
...
- Our MiniFlash reproduction preserves model quality (PPL unchanged) and matches the memory-efficient behavior, while remaining fully transparent and hackable.
